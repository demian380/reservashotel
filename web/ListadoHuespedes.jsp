...
<html lang="es">
<head>
<title>Hotel - Consulta de Huespedes</title>
<meta charset="utf-8">
...
<script type="text/javascript"> 
    ...
    function ConfirmDelete(){
        var respuesta = confirm("Está seguro de eliminar el Huésped?");
        if(respuesta == true){
            return true;
        }else{
            return false;
...
<body>
    ...
    <div class="content">
          ...
          <div class="container">
            <h2 class="top-1 p3">Huespedes del Hotel</h2>
                <div class="table-responsive custom-table-responsive">
                    ...
                    <table class="table custom-table">
                      ...
                      <tbody>
                          ...
                          <% 
                              List<Huesped> huespedes = (List) misesion.getAttribute("huespedes_registrados"); 
                              if (!huespedes.isEmpty()){
                              SimpleDateFormat formato = new SimpleDateFormat("dd/MM/yyyy");    
                              String fecha_nac;
                                for (Huesped hue : huespedes){
                                    fecha_nac = formato.format(hue.getFecha_nac());

                          %>
                                    <tr scope="row">
                                      <th scope="row">
                                        <label class="control control--checkbox">
                                          <input type="radio" name="seleccion" id="seleccion" value="<%= hue.getId() %>"/>
                                      ...
                                      <% int id_hue = hue.getId(); %>

                                      <td>
                                          <form name="frmEditarHuesped" action="SvEditarHuesped" method="post" style="display: inline">
                                              <input type="hidden" name="id_hue" value="<%= id_hue %>">
                                              <button type="submit" class="btn btn-primary btn-xs" style="display: inline">Editar</button>
                                          </form>
                                      </td>

                                      <td>
                                          <form name="frmBorrarHuesped" action="SvEliminarHuesped" method="post" style="display: inline">
                                              <input type="hidden" name="id_hue" value="<%= id_hue %>">
                                              <button type="submit" class="btn btn-danger btn-xs" style="display: inline" onclick="return ConfirmDelete()">Eliminar</button>
                                          </form>
                                      </td> 
                        ...
                        <% } else { %>
                                    <script> alert ('No hay Huespedes Registrados.') </script>
                                </tbody>
                            </table>
                        <input class="button" type="submit" value="Volver"/>
                        <% } %>
